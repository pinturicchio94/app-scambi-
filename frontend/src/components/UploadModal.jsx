...
import {
  ...
  Image as ImageIcon, X, Plus, Loader2, PenLine
} from "lucide-react";
...
export default function UploadModal({ open, onOpenChange }) {
  ...
  // Photos
  const [photos, setPhotos] = useState([]); // [{file, previewUrl, uploadedUrl}]
  ...
  const resetForm = useCallback(() => {
    ...
    setAiDone(false);
    setAiData(null);
    setName(""); setCategory(""); setSubcategory(""); setTags([]);
    setTagInput(""); setEstimatedValue(""); setCondition("");
    setDescription(""); setTransactionType("scambio");
    setDesiredTradeFor(""); setNotes("");
  ...
  const handleFilesSelect = async (files) => {
    ...
    for (let i = photos.length; i < updatedPhotos.length; i++) {
      const url = await uploadSingleFile(updatedPhotos[i].file);
      updatedPhotos[i] = { ...updatedPhotos[i], uploadedUrl: url };
    }
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-xl max-h-[90vh] overflow-y-auto" data-testid="upload-modal">
        ...
        {done ? (
          ...
          /* STEP 1: Photo Upload + AI Recognition */
            ...
            <div className="grid grid-cols-3 gap-3">
              {photos.map((photo, i) => (
                <div key={i} className="relative aspect-square rounded-xl overflow-hidden bg-gray-50 border border-gray-200">
                  ...
                  {i === 0 && (
                    <span className="absolute bottom-1.5 left-1.5 text-[9px] bg-yellow-400 text-yellow-900 font-bold px-1.5 py-0.5 rounded">
                      Principale
                    </span>
                  )}
                </div>
              ...
              {photos.length < 6 && (
                <label
                  className="aspect-square rounded-xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center cursor-pointer hover:border-gray-400 transition-colors"
                ...
                >
                  ...
                  <span className="text-[10px] text-gray-400">
                    {photos.length === 0 ? "Aggiungi foto" : "Altre foto"}
                  </span>
                </label>
              )}
            </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input
                ...
                placeholder="Es. Charizard Holo 1a Edizione"
                className="mt-1"
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="text-[10px] uppercase tracking-wider text-gray-500 font-medium">Sottocategoria</label>
                <Select value={subcategory} onValueChange={setSubcategory}>
                  <SelectTrigger className="mt-1" data-testid="subcategory-select">
                    <SelectValue placeholder="Sotto" />
                  </SelectTrigger>
                  <SelectContent>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5 mt-1.5 mb-2">
                {tags.map((tag) => (
                  <Badge key={tag} variant="secondary" className="text-[10px] bg-gray-100 text-gray-600 border-0 pr-1 cursor-pointer hover:bg-gray-200"
              ...
              <div className="flex gap-2">
                ...
                />
                <Button onClick={addTag} variant="outline" size="sm" className="px-3" data-testid="add-tag-btn">
          ...
          /* STEP 3: Condition, Trade Preferences, Submit */
          <div className="space-y-5">
            <div className="grid grid-cols-2 gap-3">
              <div>
              ...
              <div>
                ...
                <Select value={transactionType} onValueChange={setTransactionType}>
                  <SelectTrigger className="mt-1" data-testid="transaction-type-select">
            ...
            {/* Desired Trade For (only if scambio) */}
            {transactionType === "scambio" && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
                <label className="text-[10px] uppercase tracking-wider text-yellow-700 font-semibold block mb-1.5">
                  Scambio desiderato (opzionale)
                </label>
                <p className="text-[11px] text-yellow-700/70 mb-2">
                  Specifica con cosa vorresti scambiare questo oggetto
                </p>
                <Input
                  ...
                  className="bg-white border-yellow-200 focus:ring-yellow-400"
            ...
            <div className="bg-gray-50 rounded-xl p-4 text-sm space-y-1">
              ...
              <p className="text-gray-500 text-xs">{category}{subcategory ? ` / ${subcategory}` : ""} - {condition || "N/D"}</p>
